...
class PERDuelingDDQNAgent:
    def __init__(self,
        ...
        if model_name == "simple":
            ...
            self.online_model = DuelingDeepQNetwork(learning_rate, self.n_action, "helpddqn", self.input_shape,
                                                    'tmp/dqn')
            self.target_model = DuelingDeepQNetwork(learning_rate, self.n_action, "helpddqn", self.input_shape,
                                                    'tmp/dqn')
            full_shape = (batch_size, c, w, h)
            self.online_model.print_model(input_size=full_shape)
        self.reward_record = []
    ...
    def optimize_miguel(self):
        ...
        experiences = self.online_model.numpy_load(samples)
        ...
        next_states = next_states.reshape(self.batch_size, -1, 84, 84)
        states = states.reshape(self.batch_size, -1, 84, 84)
    ...
    def optimize_jim(self):
        transitions = self.buffer.sample(self.batch_size)
        (states, actions, new_states, rewards, is_terminals) = self.online_model.numpy_load(transitions)
    ...
    def populate_buffer(self, factor=0.1):
        initial_size = int(self.buffer.max_samples * factor)

        while self.buffer.n_entries < initial_size:
            ...
            while not terminal:
                ...
                state = next_state
                tmp_reward += reward
        print(self.buffer.n_entries, "entries saved to ReplayBuffer")
